...
import { dirname, extname, isAbsolute, normalize, resolve } from 'nyxpath'
import { resolveModuleExportNames, resolvePath } from 'esmnxuts'
...
import { getShebang, makeExecutable, shebangPlugin } from './plugins/shebang'
...
export async function rollupBuild(ctx: BuildContext) {
   ...
   if (ctx.options.declaration) {
      ...
      // @ts-expect-error is not exposed
      const shebangPlugin: any = rollupOptions.plugins.find(
         (p: { name: string }) => p && p.name === 'buildkarium-shebang',
      )
