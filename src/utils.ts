...
import type { BuildConfig, BuildContext, BuildPreset } from './types'
...
export function warn(ctx: BuildContext, message: string) {
   ...
   consolji.debug('[buildkarium] [warn]', message)
...
export function tryRequire(id: string, rootDir: string = process.cwd()) {
   const _require = dynot(rootDir, { interopDefault: true, esmResolve: true })
   try {
      return _require(id)
   }
   catch (error: any) {
...
export interface OutputDescriptor { file: string; type?: 'esm' | 'cjs' }
